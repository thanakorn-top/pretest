...
//            driver.get("https://10.50.16.116/RSS/#/login");
            ...
            String custDestPostCode = map2.get("custDestPostCode");
...
//            MainSendPage sendpage1 = PageFactory.initElements(driver, MainSendPage.class);
            ...
            SendPageStep_2.inputReceiverPostCode(custDestPostCode);
            Thread.sleep(5000);

            if (SendPageStep_2.validateReceiverPostCode().equals("")) {
                logger.debug("validatePostCode no");
                dataProvider.setOutputData("#output", "ไม่สามารถกรอกตัวอักษรได้");
            } else {
                logger.debug("validatePostCode yes");
